...
async def test_validator_rejection_does_not_advance_call_state(make_session: MakeSession):
    ...
    try:
        ...
        # submit_and_await_turn for the first (not back-to-back submits) so the
        # two rejections land as separate turns instead of coalescing into one —
        # the watchdog needs two no-progress turns to trip.
        menu = "Press 1 for sales, press 2 for billing, press 3 for claims"
        await submit_and_await_turn(runner, menu)
        runner.submit_transcript(menu)
        await wait_until(lambda: runner.session.done)
